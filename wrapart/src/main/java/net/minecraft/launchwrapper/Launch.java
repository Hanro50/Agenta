...
public class Launch {
    ...
    private void launch(String[] args) {
        ...
        try {
            ...
            do {
                for (final Iterator<String> it = tweakClassNames.iterator(); it.hasNext(); ) {
                    ...
                    if (allTweakerNames.contains(tweakName)) {
                        Prt.warn( "Tweak class name %s has already been visited -- skipping", tweakName);
                    ...
                    Prt.warn("Loading tweak class name %s", tweakName);
                    ...
                    if (primaryTweaker == null) {
                        Prt.warn( "Using primary tweak class name %s", tweakName);
                ...
                for (final Iterator<ITweaker> it = tweakers.iterator(); it.hasNext(); ) {
                    ...
                    Prt.warn("Calling tweak class %s", tweaker.getClass().getName());
            ...
            Prt.log(LEVEL.INFO,"Launching wrapped minecraft {%s}", launchTarget);
            mainMethod.invoke(null, (Object) argumentList.toArray(new String[argumentList.size()]));
        } catch (Exception e) {
            e.printStackTrace();
            Prt.warn("Unable to launch");
            System.exit(1);
        }
    }
}
