...
public class LaunchClassLoader extends URLClassLoader {
    ...
    public LaunchClassLoader(URL[] sources) {
        ...
        if (DEBUG_SAVE) {
            ...
            if (tempFolder.exists()) {
                Prt.info("DEBUG_SAVE enabled, but 10 temp directories already exist, clean them and try again.");
                tempFolder = null;
            } else {
                Prt.info("DEBUG_SAVE Enabled, saving all classes to \"%s\"", tempFolder.getAbsolutePath().replace('\\', '/'));
                tempFolder.mkdirs();
            }
        }
    }
    ...
    public void registerTransformer(String transformerClassName) {
        try {
            ...
            e.printStackTrace();
            Prt.warn("A critical problem occurred registering the ASM transformer class %s", transformerClassName);
        }
    }
    ...
    public Class<?> findClass(final String name) throws ClassNotFoundException {
        ...
        try {
            ...
            if (lastDot > -1 && !untransformedName.startsWith("net.minecraft.")) {
                if (urlConnection instanceof JarURLConnection) {
                    ...
                    if (jarFile != null && jarFile.getManifest() != null) {
                        ...
                        if (pkg == null) {
                            ...
                            if (pkg.isSealed() && !pkg.isSealed(jarURLConnection.getJarFileURL())) {
                                Prt.log(LEVEL.FETAL,"The jar file %s is trying to seal already secured path %s", jarFile.getName(), packageName);
                            } else if (isSealed(packageName, manifest)) {
                                Prt.log(LEVEL.FETAL,"The jar file %s has a security seal for path %s, but that path is defined and not secure", jarFile.getName(), packageName);
                            }
                        }
                    }
                } else {
                    ...
                    if (pkg == null) {
                        ...
                        Prt.log(LEVEL.FETAL,"The URL %s is defining elements for sealed path %s", urlConnection.getURL(), packageName);
                    }
                }
            }
            ...
            if (DEBUG) {
                e.printStackTrace();
                Prt.warn("Exception encountered attempting classloading of %s", name);
            }
    ...
    private void saveTransformedClass(final byte[] data, final String transformedName) {
        ...
        try {
            Prt.log(LEVEL.FETAL,"Saving transformed class \"%s\" to \"%s\"", transformedName, outFile.getAbsolutePath().replace('\\', '/'));
            ...
            ex.printStackTrace();
            Prt.warn("Could not save transformed class \"%s\"", transformedName);
        }
    }
    ...
    private byte[] runTransformers(final String name, final String transformedName, byte[] basicClass) {
        if (DEBUG_FINER) {
            Prt.log(LEVEL.INFO,"Beginning transform of {%s (%s)} Start Length: %d", name, transformedName, (basicClass == null ? 0 : basicClass.length));
            for (final IClassTransformer transformer : transformers) {
                final String transName = transformer.getClass().getName();
                Prt.log(LEVEL.INFO,"Before Transformer {%s (%s)} %s: %d", name, transformedName, transName, (basicClass == null ? 0 : basicClass.length));
                basicClass = transformer.transform(name, transformedName, basicClass);
                Prt.log(LEVEL.INFO,"After  Transformer {%s (%s)} %s: %d", name, transformedName, transName, (basicClass == null ? 0 : basicClass.length));
            }
            Prt.log(LEVEL.INFO,"Ending transform of {%s (%s)} Start Length: %d", name, transformedName, (basicClass == null ? 0 : basicClass.length));
        } else {
            for (final IClassTransformer transformer : transformers) {
                basicClass = transformer.transform(name, transformedName, basicClass);
            }
    ...
    private byte[] readFully(InputStream stream) {
        try {
            ...
            t.printStackTrace();
            Prt.warn( "Problem loading class");
            return new byte[0];
        }
    }
    ...
    public byte[] getClassBytes(String name) throws IOException {
        ...
        try {
            ...
            if (classResource == null) {
                if (DEBUG) Prt.log(LEVEL.INFO,"Failed to find class resource %s", resourcePath);
                negativeResourceCache.add(name);
                return null;
            }
            classStream = classResource.openStream();

            if (DEBUG) Prt.log(LEVEL.INFO,"Loading class %s from resource %s", name, classResource.toString());
